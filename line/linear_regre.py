...
def linear_gradient():
    ...
    w=np.matrix(w)
    len_test = np.shape(y_train)[0]
    y_train=np.dot(y_train,1)
    y_train=np.reshape(y_train,(len_test,1))
    ...
    while True:
        ...
        epoch=epoch+1
        if math.fabs( last_total_error - total_error ) < 0.000000001:
            # 总误差变化很小，退出训练
